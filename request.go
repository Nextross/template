...
func (p *Provider) buildRequest(ctx context.Context, command string, clTRID string, payload any) (request *http.Request, err error) {
	if p.Username == "" || p.Password == "" {
		err = fmt.Errorf("buildRequest: missing username and/or password")
		return
	}

	request, err = http.NewRequestWithContext(ctx, http.MethodPost, BaseURL, nil)
	if err != nil {
		err = fmt.Errorf("buildRequest: failed to create request: %v", err)
		return
	}
	...
	if err != nil {
		return nil, fmt.Errorf("buildRequest: failed to marshal request: %v", err)
	}
	...
	request.Body = io.NopCloser(requestEncoded)
